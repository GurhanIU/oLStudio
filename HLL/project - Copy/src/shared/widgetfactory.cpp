...
//#include "elayout_widget_p.h"
//#include "edesigner_widget_p.h"
//#include "edesigner_tabwidget_p.h"
//#include "edesigner_toolbox_p.h"
//#include "edesigner_stackedbox_p.h"
//#include "edesigner_toolbar_p.h"
//#include "edesigner_menubar_p.h"
//#include "edesigner_menu_p.h"
//#include "edesigner_dockwidget_p.h"
...
{
    // Is it one of  EDesignerAxWidget/EDesignerAxPluginWidget?
    static const char *axWidgetName = "EDesignerAx";
...
void WizardPageChangeWatcher::pageChanged()
{
    ...
    EDesignerFormWindowInterface *fw = EDesignerFormWindowInterface::findFormWindow(wizard);
    if (!fw)
        return;
//    EDesignerFormWindowCursorInterface *cursor = fw->cursor();
...
WidgetFactory::Strings::Strings() :
    ...
    m_eLayoutWidget(QLatin1String("ELayoutWidget")),
...
WidgetFactory::WidgetFactory(EDesignerFormEditorInterface *core, QObject *parent)
    : EDesignerWidgetFactoryInterface(parent),
      m_core(core),
      m_formWindow(0),
      m_currentStyle(0)
{
...
EDesignerFormWindowInterface *WidgetFactory::currentFormWindow(EDesignerFormWindowInterface *fw)
{
    EDesignerFormWindowInterface *was = m_formWindow;
    m_formWindow = fw;
    return was;
}
...
//    EDesignerPluginManager *pluginManager = m_core->pluginManager();

//    QList<EDesignerCustomWidgetInterface*> lst = pluginManager->registeredCustomWidgets();
//    foreach (EDesignerCustomWidgetInterface *c, lst)
...
//    EDesignerCustomWidgetInterface *factory = it.value();
...
//    {
//        EDesignerWidgetDataBaseInterface *wdb = m_core->widgetDataBase();
//        const int widgetInfoIndex = wdb->indexOfObject(rc, false);
//        if (widgetInfoIndex != -1)
//        {
//            if (wdb->item(widgetInfoIndex)->extends().isEmpty())
//            {
//                const EDesignerMetaObjectInterface *mo = core()->introspection()->metaObject(rc)->superClass();
...
//    EDesignerLanguageExtension *lang = e_extension<EDesignerLanguageExtension *>(m_core->extensionManager(), m_core);
//    if (lang)
//        return rc;
...
QWidget *WidgetFactory::createWidget(const QString &widgetName, QWidget *parentWidget) const
{
    ...
    EDesignerFormWindowInterface *fw = m_formWindow;
    if (! fw)
        fw = EDesignerFormWindowInterface::findFormWindow(parentWidget);
...
//        }
        ...
        // 2) EDesigner Special widgets
//        if (widgetName == m_strings.m_line)
//            w = new Line(parentWidget);
//        else if (widgetName == m_strings.m_qDockWidget)
//            w = new EDesignerDockWidget(parentWidget);
//        else if (widgetName == m_strings.m_qMenuBar)
//            w = new EDesignerMenuBar(parentWidget);
//        else if (widgetName == m_strings.m_qMenu)
//            w = new EDesignerMenu(parentWidget);
//        else if (widgetName == m_strings.m_spacer)
//            w = new Spacer(parentWidget);
//        else if (widgetName == m_strings.m_qDockWidget)
//            w = new EDesignerDockWidget(parentWidget);
//        else if (widgetName == m_strings.m_eLayoutWidget)
//            w = fw ? new ELayoutWidget(fw, parentWidget) : new QWidget(parentWidget);
//        else if (widgetName == m_strings.m_qDialog)
//        {
//            if (fw)
//                w = new EDesignerDialog(fw, parentWidget);
//            else
//                w = new QDialog(parentWidget);
//        }
//        else if (widgetName == m_strings.m_qWidget)
//        {
//            /* We want a 'EDesignerWidget' that draws a grid only for widget
//             * forms and container extension pages (not for preview and not
//             * for normal QWidget children on forms (legacy) */
//            if (fw && parentWidget)
//            {
//                if (e_extension<EDesignerContainerExtension*>(m_core->extensionManager(), parentWidget))
//                    w = new EDesignerWidget(fw, parentWidget);
//                else
//                {
...
//        EDesignerWidgetDataBaseInterface *db = core()->widgetDataBase();
//        EDesignerWidgetDataBaseItemInterface *item = db->item(db->indexOfClassName(widgetName));
//        if (item == 0)
//        {
...
QString WidgetFactory::classNameOf(EDesignerFormEditorInterface *c, const QObject* o)
{
    if (o == 0)
        return QString();
...
//    if (qobject_cast<const EDesignerMenuBar*>(w))
//        return QLatin1String("QMenuBar");
//    else if (qobject_cast<const EDesignerMenu*>(w))
//        return QLatin1String("QMenu");
//     else if (qobject_cast<const EDesignerDockWidget*>(w))
//        return QLatin1String("QDockWidget");
//    else if (qobject_cast<const EDesignerDialog*>(w))
//        return QLatin1String("QDialog");
//    else if (qobject_cast<const EDesignerWidget*>(w))
//        return QLatin1String("QWidget");
//#ifdef Q_OS_WIN
//    else if (isAxWidget(w))
//        return QLatin1String("QAxWidget");
//#endif
//    else if (qstrcmp(className, "EDesignerQ3WidgetStack") == 0)
...
//{
//    EDesignerMetaDataBaseInterface *metaDataBase = core()->metaDataBase();
...
//    EDesignerPropertySheetExtension *sheet = e_extension<EDesignerPropertySheetExtension*>(core()->extensionManager(), layout);
...
//    }
//    else if (widget->inherits("ELayoutWidget"))
//    {
...
//        if (!box)
//        {  // we support only unmanaged box layouts
...
//{
//    if (EDesignerContainerExtension *container = e_extension<EDesignerContainerExtension*>(core()->extensionManager(), w))
...
//    while (w != 0)
//    {
//        if (core()->widgetDataBase()->isContainer(w) || (w && qobject_cast<EDesignerFormWindowInterface*>(w->parentWidget())))
//            return w;
...
EDesignerFormEditorInterface *WidgetFactory::core() const
{
    return m_core;
}
...
//    {
//        EStackedWidgetPreviewEventFilter::install(stackedWidget); // Add browse button only.
//        return;
//    }
}
...
{
    // Indicate that this is a form object (for EDesignerFormWindowInterface::findFormWindow)
//    object->setProperty(formEditorDynamicProperty, QVariant(true));
//    EDesignerPropertySheetExtension *sheet = e_extension<EDesignerPropertySheetExtension*>(m_core->extensionManager(), object);
//    if (!sheet)
//        return;
...
//    {
//        EToolBoxHelper::install(toolBox);
//        return;
//    }
//    if (QStackedWidget *stackedWidget = qobject_cast<QStackedWidget*>(widget))
//    {
//        EStackedWidgetEventFilter::install(stackedWidget);
//        return;
//    }
//    if (QTabWidget *tabWidget = qobject_cast<QTabWidget*>(widget))
//    {
//        ETabWidgetEventFilter::install(tabWidget);
//        return;
//    }
...
void WidgetFactory::formWindowAdded(EDesignerFormWindowInterface *formWindow)
{
    setFormWindowStyle(formWindow);
}

void WidgetFactory::activeFormWindowChanged(EDesignerFormWindowInterface *formWindow)
{
    setFormWindowStyle(formWindow);
}

void WidgetFactory::setFormWindowStyle(EDesignerFormWindowInterface *formWindow)
{
