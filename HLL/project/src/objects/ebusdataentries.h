...
class EBusDataEntries : public QObject
{
    Q_OBJECT
public:
    explicit EBusDataEntries(QObject *parent = nullptr)
        : EBusDataEntries(nullptr, 0, parent) { }
    ...
    EBusData *addEntry(int device, int registerId, int address, QVariant data, const QString &alias = QString());
    EBusData *addEntry(int registerId, int address, QVariant data, const QString &alias);
