 ...
 */
EBusData * EBusDataEntries::addEntry(int device, int registerId, int address, QVariant data, const QString &alias)
{
    EBusData *entry;
    if (!hasEntry(address)) {
        entry = new EBusData(device, m_registerType, registerId, address, data, this);
...
EBusData* EBusDataEntries::addEntry(int registerId, int address, QVariant data, const QString &alias)
{
    return addEntry(m_device, registerId, address, data, alias);
}
    ...
    if (reply->error() == QModbusDevice::NoError) {
        ...
        for (uint i = 0; i < unit.valueCount(); i++) {
            ...
            if (busData) {
                ...
                qDebug() << "AA" << busData->alias() << busData->toString();
                qDebug() << "BB" << busData->alias() << busData->toFormattedString();

//                QString s = QString("123456");
//                const QByteArray ar = s.toUtf8();

//                modbusData->setData(modbusData->dataType(), (void*)ar.data());
            }
        }
...
QList<QModbusDataUnit> EBusDataEntries::prepareDataUnitsForWrite(const QList<EntryList *> &entries)
{
    ...
    foreach (EntryList *entry, entries) {
        ...
        for (int i=0; i < entry->count(); i++)
            writeUnit.setValue(i, entry->at(i)->data().toUInt());
