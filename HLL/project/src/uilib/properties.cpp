...
// Convert complex DOM types with the help of  EAbstractFormBuilder
QVariant domPropertyToVariant(EAbstractFormBuilder *afb,const QMetaObject *meta,const  DomProperty *p)
{
    // Complex types that need functions from EAbstractFormBuilder
    switch(p->kind()) {
    ...
    case DomProperty::Enum: {
        ...
        if (index == -1)
        {
            ...
            if (!qstrcmp(meta->className(), "QFrame") && (pname == QByteArray("orientation")))
                return QVariant(enumValue == EFormBuilderStrings::instance().horizontalPostFix ? QFrame::HLine : QFrame::VLine);
            else
            {
...
QVariant domPropertyToVariant(const DomProperty *p)
{
    ...
    case DomProperty::Bool:
        return QVariant(p->elementBool() == EFormBuilderStrings::instance().trueValue);
    ...
    case DomProperty::Font: {
        ...
        if (font->hasElementStyleStrategy()) {
            f.setStyleStrategy(enumKeyOfObjectToValue<EAbstractFormBuilderGadget, QFont::StyleStrategy>("styleStrategy", font->elementStyleStrategy().toLatin1()));
        }
    ...
    case DomProperty::CursorShape:
        return QVariant::fromValue(QCursor(enumKeyOfObjectToValue<EAbstractFormBuilderGadget, Qt::CursorShape>("cursorShape", p->elementCursorShape().toLatin1())));
#endif

    case DomProperty::Locale: {
        const DomLocale *locale = p->elementLocale();
        return QVariant::fromValue(QLocale(enumKeyOfObjectToValue<EAbstractFormBuilderGadget, QLocale::Language>("language", locale->attributeLanguage().toLatin1()),
                    enumKeyOfObjectToValue<EAbstractFormBuilderGadget, QLocale::Country>("country", locale->attributeCountry().toLatin1())));
    }
    case DomProperty::SizePolicy: {
        ...
        const QMetaEnum sizeType_enum = metaEnum<EAbstractFormBuilderGadget>("sizeType");
...
static bool applySimpleProperty(const QVariant &v, bool translateString, DomProperty *dom_prop)
{
    ...
    case QVariant::Bool:
        dom_prop->setElementBool(v.toBool() ? EFormBuilderStrings::instance().trueValue : EFormBuilderStrings::instance().falseValue);
        return true;
    ...
    case QVariant::Font: {
        ...
        {
            const QMetaEnum styleStrategy_enum = metaEnum<EAbstractFormBuilderGadget>("styleStrategy");
    ...
    case QVariant::Cursor: {
        const QMetaEnum cursorShape_enum = metaEnum<EAbstractFormBuilderGadget>("cursorShape");
    ...
    case QVariant::Locale: {
        ...
        const QMetaEnum language_enum = metaEnum<EAbstractFormBuilderGadget>("language");
        const QMetaEnum country_enum = metaEnum<EAbstractFormBuilderGadget>("country");
...
// Convert complex variant types to DOM properties with the help of  EAbstractFormBuilder
// Does not perform a check using  EAbstractFormBuilder::checkProperty().
DomProperty *variantToDomProperty(EAbstractFormBuilder *afb, const QMetaObject *meta,
