...
WidgetBoxTreeWidget::WidgetBoxTreeWidget(EDesignerFormEditorInterface *core, QWidget *parent) :
    QTreeWidget(parent),
    m_core(core),
    m_iconMode(false),
...
void WidgetBoxTreeWidget::addCategory(const WidgetBoxTreeWidget::Category &cat)
{
...
/*void WidgetBoxTreeWidget::dropWidgets(const QList<EDesignerDnDItemInterface *> &item_list)
{
...
bool WidgetBoxTreeWidget::load(EDesignerWidgetBox::LoadMode loadMode)
{
    switch (loadMode) {
    case EDesignerWidgetBox::LoadReplace:
        clear();
        break;
    case EDesignerWidgetBox::LoadCustomWidgetsOnly:
...
bool WidgetBoxTreeWidget::loadContents(const QString &contents)
{
    ...
    {
        edesigner_internal::designerWarning(errorMessage);
        return false;
    }
...
WidgetBoxCategoryListView *WidgetBoxTreeWidget::categoryViewAt(int idx) const
{
    ...
    if (QTreeWidgetItem *cat_item = topLevelItem(idx))
    {
        if (QTreeWidgetItem *embed_item = cat_item->child(0))
            rc = qobject_cast<WidgetBoxCategoryListView*>(itemWidget(embed_item, 0));
        Q_ASSERT(rc);
        return rc;
    }
}
...
void WidgetBoxTreeWidget::adjustSubListSize(QTreeWidgetItem *cat_item)
{
    ...
    WidgetBoxCategoryListView *list_widget = static_cast<WidgetBoxCategoryListView*>(itemWidget(embed_item, 0));
...
{
    return EDesignerWidgetBox::tr("An error has been encountered at line %1 of %2: %3")
...
//bool WidgetBoxTreeWidget::readWidget(Widget *w, const QString &xml, QXmlStreamReader &r)
{
    ...
    while (!endEncountered) {
            ...
            if (nesting++ == 0) {
                ...
                if (name == QLatin1String(uiElementC)) {
                    ...
                    if (name == QLatin1String(widgetElementC)) {
                        ...
                        r.raiseError(EDesignerWidgetBox::tr("Unexpected element <%1> encountered when parsing for <widget> or <ui>").arg(name.toString()));
                        return false;
                    }
                }
            } else {
        ...
        case QXmlStreamReader::EndDocument:
            r.raiseError(EDesignerWidgetBox::tr("Unexpected end of file encountered when parsing widgets."));
            return false;
    ...
    if (!parsedWidgetTag) {
        r.raiseError(EDesignerWidgetBox::tr("A widget element could not be found."));
        return false;
    }
