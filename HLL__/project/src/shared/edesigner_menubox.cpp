...
EMenuBox::EMenuBox(QWidget *parent, Qt::WindowFlags flags)
    : EMenuBoxInterface(parent, flags),
      m_loadMode(LoadMerge),
      m_pageChangedForwardingBlocked(false)
{

}

EMenuBox::LoadMode EMenuBox::loadMode() const
{
    return m_loadMode;
}

void EMenuBox::setLoadMode(LoadMode lm)
{
    m_loadMode = lm;
}

// Convience to find a widget by class name
bool EMenuBox::findPage(const EMenuBoxInterface *wbox,
...
//DomUI *EDesignerWidgetBox::xmlToUi(const QString &name, const QString &xml, bool insertFakeTopLevel, QString *errorMessage)
//{
...
//            if (name.compare(QLatin1String("widget"), Qt::CaseInsensitive) == 0)
...
//            }
//            else if (name.compare(QLatin1String("ui"), Qt::CaseInsensitive) == 0)
//            { // 4.4
//                ui = new DomUI;
//                ui->read(reader);
//            }
...
//// Convenience to create a Dom Widget from widget box xml code.
//DomUI *EDesignerWidgetBox::xmlToUi(const QString &name, const QString &xml, bool insertFakeTopLevel)
//{
...
void EMenuBox::emitPageChanged(const QString &name, const int &id)
{
    // Avoid duplicate signal emission - see below
    m_pageChangedForwardingBlocked = true;
    emit pageChanged(name, id);
    m_pageChangedForwardingBlocked = false;
}

void EMenuBox::slotPageChanged(const QString &name, const int &id)
{
    // Forward signal from Integration using the old interfaces.
    if (!m_pageChangedForwardingBlocked)
        emit pageChanged(name, id);
}
